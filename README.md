Cloned and modified https://github.com/Cardinal-Space-Mining/direct_lidar_odometry
...
- Ubuntu 20.04 or 22.04 (tested on 22.04 Humble)
- ROS Noetic or Humble (`rclcpp`, `std_msgs`, `sensor_msgs`, `geometry_msgs`)
- C++ 14
- CMake >= `3.16.3`
- OpenMP >= `4.5`
...
```sh
mkdir ros2_ws && cd ws && mkdir src && cd src
git clone https://github.com/vivaad/direct_lidar_odometry.git
cd ..
colcon build
```
...
ros2 launch direct_lidar_odometry dlo.launch \
  ...
  imu_topic:=/imu # give your own pointcloud and imu topics
```
...
```sh
ros2 service call /robot/dlo_map/save_pcd LEAF_SIZE SAVE_PATH
```
To save the trajectory in KITTI format, call the following service:

```sh
ros2 service call /robot/dlo_odom/save_traj SAVE_PATH
```
